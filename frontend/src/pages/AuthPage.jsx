...
export default function AuthPage({ mode, onNav, onSuccess }) {
    ...
    const submit = async () => {
        ...
        if (!email.toLowerCase().endsWith("@gmail.com")) { setErr("MUST USE A @GMAIL.COM ADDRESS"); setShake(true); setTimeout(() => setShake(false), 400); return; }
    ...
    return (
        <div style={{ minHeight: "100vh", display: "flex", alignItems: "center", justifyContent: "center", position: "relative" }}>
            ...
            <div style={{ position: "relative", zIndex: 2, width: "100%", maxWidth: 440, padding: 24 }}>
                <CyberCard style={{ padding: 40 }} color="var(--cyan)">
                    ...
                    <div className="MONO" style={{ fontSize: 12, color: "rgba(var(--cyan-rgb),.4)", textAlign: "center", marginBottom: 32, letterSpacing: ".2em" }}>
                    ...
                    <div className={shake ? "aShake" : ""} style={{ display: "flex", flexDirection: "column", gap: 16 }}>
                        <div>
                            <div className="MONO" style={{ fontSize: 11, color: "rgba(var(--cyan-rgb),.5)", marginBottom: 6, letterSpacing: ".15em" }}>EMAIL_ADDRESS</div>
                            <input value={email} onChange={(e) => setEmail(e.target.value)} placeholder="coder@guild.net" onKeyDown={(e) => e.key === "Enter" && submit()} />
                        </div>
                        <div>
                            <div className="MONO" style={{ fontSize: 11, color: "rgba(var(--cyan-rgb),.5)", marginBottom: 6, letterSpacing: ".15em" }}>ACCESS_CODE</div>
                    ...
                    <div style={{ marginTop: 24, textAlign: "center" }}>
                        <span className="MONO" style={{ fontSize: 12, color: "rgba(var(--text-muted-rgb),.5)" }}>{mode === "login" ? "NEW RECRUIT? " : "ALREADY ENLISTED? "}</span>
